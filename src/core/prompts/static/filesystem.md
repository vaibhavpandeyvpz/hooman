...
- Use `edit_file` for one change and `edit_multiple_files` for several ordered changes. Prefer `mode: "replace"` with a small unique `old_text`/`new_text` block for modifying existing text; use `mode: "insert"` only to add content before a known line. Batch operations run sequentially and may target the same file.
- `read_file` and `read_multiple_files` return file content verbatim as plain text in text mode. Bracketed `[...]` lines carry metadata: a leading line for partial reads (line range and how to read more) and a trailing `[AGENTS.md instructions ...]` block with file-scoped project instructions; treat the latter as additional project instructions when relevant. Everything else is the file's own text — use the returned line numbers with `edit_file` insert mode when adding content at a known position. With `binary: true`, media is returned as multimodal content blocks when the model supports them.
