...
- `mlx`: runs MLX-format models in-process on Apple Silicon via `@mlx-node/lm` (Metal GPU; macOS 26+ for the prebuilt binaries); weights are downloaded from the Hugging Face Hub into `~/.hooman/cache/huggingface` on first use. Supported architectures: Qwen3, Qwen3.5 (dense and MoE), Gemma 4, LFM2.5 — the repo must be MLX format (e.g. `mlx-community/...` or mlx-node conversions), and quantized checkpoints must use a quantization layout mlx-node understands (its own conversions do; mlx-community uniform-quant repos such as `...-4bit` currently fail with weight-shape/missing-weight errors — use `-bf16` repos or mlx-node-converted quants). Provider `options` support optional `hfToken` (Hugging Face access token for gated/private repos; falls back to the `HF_TOKEN` env var) and optional `reasoning` (an object with optional `effort`). Providing `reasoning` enables thinking (the model thinks naturally with `effort` capping thought tokens via a budget: `minimal`→1024, `low`→2048, `medium`→4096, `high`→8192, default `medium`); omitting it disables thinking (the chat template closes the think block immediately and reasoning content is dropped). LLM `options.model` accepts a Hugging Face repo (`owner/repo`) or a local MLX model directory (containing `config.json` + safetensors); the out-of-the-box config ships one entry — `mlx-community/Qwen3-0.6B-bf16` ("Qwen3 0.6B (MLX)", not the default); `temperature` and `maxTokens` are supported.
