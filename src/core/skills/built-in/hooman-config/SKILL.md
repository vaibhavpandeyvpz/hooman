...
- `anthropic`: provider `options` support `apiKey`, optional `baseURL`, optional `headers`, and optional `reasoning` (an object with optional `effort` — `"minimal"`, `"low"`, `"medium"`, `"high"`). Setting `reasoning.effort` enables extended thinking (sent as `thinking: { type: "enabled" }`; `budget_tokens` is not sent, so the proxy/model default applies); omit to keep thinking off. LLM `options` support `model`, `temperature`, and `maxTokens`.
...
- `minimax`: provider `options` support `apiKey`, optional `headers`, and optional `reasoning` (an object with optional `effort` — `"minimal"`, `"low"`, `"medium"`, `"high"`). Setting `reasoning.effort` enables thinking, normalized to MiniMax's `thinking: { type: "adaptive" }`; omit to keep thinking off. Hooman routes this through the Anthropic-compatible MiniMax endpoint automatically.
