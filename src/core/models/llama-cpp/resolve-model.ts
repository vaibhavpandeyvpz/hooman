...
/** Default Hugging Face GGUF model used when no model is configured. */
export const DEFAULT_LLAMA_CPP_MODEL = "Qwen/Qwen3-1.7B-GGUF:Q8_0";

/**
 * Secondary out-of-the-box model. Uses the unsloth conversion: Google's
 * official `google/gemma-4-E2B-it-qat-q4_0-gguf` ships a malformed metadata
 * entry (empty key) that llama.cpp's GGUF parser rejects with
 * `GGML_ASSERT(!key.empty())`, aborting the process.
 */
export const DEFAULT_LLAMA_CPP_GEMMA_MODEL = "unsloth/gemma-4-E2B-it-GGUF:Q8_0";
