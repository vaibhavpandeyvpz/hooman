...
- **Token-usage footer** with cumulative input / cached / output token totals for the session, plus a context-window gauge and cumulative session cost when the agent can resolve the model's billing metadata (from the LLM config's `billing` block or the models.dev catalog).
