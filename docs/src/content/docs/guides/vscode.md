...
- **Streaming chat** with full markdown rendering (including clickable links), and collapsible thinking (with a "thought for Xs · ~N tokens" summary).
...
- **Tool-call cards** with live status — shell commands stream their output into the card as they run. Failed requests that Hooman retries show a **retry card** with attempt count and backoff instead of silently failing.
...
- **Plan checklist** pinned above the transcript, updated live as the agent works through it, backed by a dedicated **Plan editor** custom view for `*.plan.md` files. Leaving plan mode (`exit_plan_mode`) always requires explicit approval, even with Yolo/auto-approve on.
- **Message actions**: copy any message, or **fork the chat** from a prior message into a new session/tab.
...
- **Attachments**: add files, folders, and images via the paperclip button, drag & drop, or paste from the clipboard. Right-click a file in the Explorer (or select text in an editor) for **Add to Hooman Chat** / **Add to New Hooman Chat** (and the selection-scoped variants) to send it straight into the panel.
...
- **Hooman: Open Settings…** — opens the native **Configuration** editor for `config.json`, scaffolding it first if needed; `mcp.json` and `instructions.md` get their own custom editors (**Hooman MCP**, **Hooman Instructions**) when opened directly, and a **Skills** panel manages installed skills.
