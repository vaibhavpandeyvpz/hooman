...
- **Marketplace metadata:** `license`/`icon` (`media/icon.png`, rendered from `docs/src/assets/logo.svg`)/`homepage`/`bugs`/`galleryBanner` are set on `src/vscode/package.json`; `src/vscode/LICENSE` is a copy of the root `LICENSE` (`vsce` looks for one inside the packaged sub-package, not the repo root). `src/vscode/CHANGELOG.md` is rendered as a Marketplace tab and should get an entry per published version.
...
Default `config.json` ships two local providers — `llama-cpp` (the active default) and `mlx` — with four LLM entries (downloaded from the Hugging Face Hub on first use): `unsloth/gemma-4-E2B-it-GGUF:Q4_K_M` (`context: 131072`) and `unsloth/Qwen3.5-2B-MTP-GGUF:Q4_K_M` (`context: 262144`) on llama-cpp — each pinned to its model's full training window via the per-LLM `options.context` (llama-cpp only; overrides the provider-level `context`, which was renamed from `contextSize` — a legacy alias in the schema still accepts the old key) — plus two entries on the `mlx` provider (Apple Silicon + macOS 26+ only): `mlx-community/gemma-4-e2b-it-OptiQ-4bit` ("Gemma 4 E2B (MLX)", `context: 131072`) and `mlx-community/Qwen3.5-2B-OptiQ-4bit` ("Qwen3.5 2B (MLX)", `context: 262144`) — each at its full training window; on mlx, `context` declares the usable window for the context-usage gauge rather than sizing an allocation, since MLX allocates KV state dynamically). The per-LLM/provider-level `context` configured on the local `llama-cpp`/`mlx` providers feeds the context gauge via `configuredLlmContext` + `resolveLlmBilling`'s `configuredContext` parameter (precedence: `billing.context` > configured context > models.dev catalog). Both local providers also accept a provider-level `promptCache` setting, but shaped differently per provider: llama-cpp's is a boolean (default `true`) that clears the context sequence's KV state before each turn when `false`; mlx's is `{ minTokens?, maxEntries?, ttl? } | false | null` — `undefined`/`null`/`false` (the default `config.json` ships `{}`) disables mlex's internal prompt-cache pool entirely (every generate call forwards `promptCache: false`), while an object (even `{}`) enables it and sizes the pool once at load time via `MlexModel.load`'s second argument, with any unset field falling back to mlex's own defaults (16 entries, 300s TTL, 8-token minimum). Do not switch the Gemma entry to Google's official `google/gemma-4-E2B-it-qat-q4_0-gguf`: that GGUF has a malformed metadata entry (empty key) that llama.cpp's parser rejects with `GGML_ASSERT(!key.empty())`, crashing the process. Supported providers include `anthropic`, `azure`, `bedrock`, `google`, `groq`, `llama-cpp`, `minimax`, `mlx`, `moonshot`, `ollama`, `openai`, `openrouter`, and `xai`. See `src/core/models/index.ts` for the currently wired providers, `src/core/config.ts` for the top-level config schema, and `src/core/models/types.ts` for provider/LLM option schemas. Reasoning-capable providers share a common `reasoning: { effort?, summary?, display? }` option that each factory translates to the backend's native shape. The `minimax` factory is served through the AI SDK Anthropic adapter (`@ai-sdk/anthropic` via the Strands `VercelModel`, default `baseURL` `https://api.minimax.io/anthropic`, overridable) rather than the native Strands `AnthropicModel`, because it reads token usage from the stream's final `message_delta` — MiniMax reports `input_tokens: 0` in `message_start`, which the native model trusts. The `llama-cpp` provider (`src/core/models/llama-cpp/`) runs GGUF models in-process via `node-llama-cpp` through a custom Strands `Model` (like Ollama's); `options.model` is a Hugging Face repo (`owner/repo`, GGUF file auto-detected; `owner/repo:QUANT` picks a quant variant; the bundled presets use `Q4_K_M`), an exact `owner/repo/file.gguf`, or a local `.gguf` path, downloaded via `@huggingface/hub` (optional `hfToken`, falling back to `HF_TOKEN`) into `~/.hooman/cache/huggingface`; loaded runtimes/weights are shared process-wide, tool calls use node-llama-cpp's chat-wrapper function calling (results are folded back into the model response as `ChatModelFunctionCall` items), and thought segments stream as reasoning. The shared `reasoning` option maps onto the resolved chat wrapper (`resolveChatWrapper` with `customWrapperSettings`): presence enables thinking (Qwen `thoughts: "auto"`, Gemma 4 `reasoning: true`, Harmony `reasoningEffort` from `effort`) with `effort` capping thought tokens via `budgets.thoughtTokens` (default `medium`); omitting `reasoning` disables it (wrappers discourage thoughts, thought budget forced to 0 so always-thinking models close their thought segment immediately). Tool inputSchemas are standard JSON Schema but node-llama-cpp only accepts its GBNF subset (no `anyOf`/`allOf`, every listed property is required), so `gbnf-schema.ts` converts them — `anyOf`→`oneOf`, optional properties become `oneOf: [<schema>, null]` and the null markers are pruned from generated params against the original schema (`pruneOptionalNulls`) before the tool call is emitted; unsupported shapes degrade to an any-JSON-value grammar instead of failing generation. The `mlx` provider (`src/core/models/mlx/`) runs MLX-format models in-process on Apple Silicon via [`mlex.js`](https://github.com/vaibhavpandeyvpz/mlex) (Metal GPU; the prebuilt binaries require macOS 26+ — the dependency is imported dynamically inside the factory so other platforms are unaffected until the provider is actually used). Supported architectures (auto-detected from `config.json` `model_type`): Qwen2/2.5, Qwen3, Qwen3.5/3.6 dense/MoE, Gemma 4 (including multi-modal vision/audio variants), Nemotron 3 (hybrid Mamba2/attention), DharaAR; every MLX quantization scheme loads (bf16/fp16, affine 2-8 bit, mxfp4/mxfp8/nvfp4, mixed-precision OptiQ/Google-QAT). `options.model` is an MLX-format Hugging Face repo (`owner/repo`, e.g. `mlx-community/...`) or a local MLX model directory, downloaded file-by-file via `@huggingface/hub` into the same `~/.hooman/cache/huggingface` cache (all files pinned to the repo's head commit sha via `modelInfo` so they land in one snapshot dir). Loaded models are shared process-wide; each `stream()` call passes the full Strands conversation to the stateless `MlexModel.generate` (OpenAI/Anthropic-style — an internal prompt-cache pool reuses KV state for any already-computed prefix), with typed streamed tokens (`kind: "text" | "reasoning" | "toolCall"`) bridged into Strands deltas — raw `toolCall`-kind deltas are skipped during streaming and the final result's parsed `toolCalls` are emitted as toolUse blocks instead. Tools pass their JSON Schema through natively (no GBNF-style conversion); usage reporting is OpenAI-style inclusive (`promptTokens` includes `cachedTokens`), handled via `markTotalInclusiveInputUsage`. The shared `reasoning` option: presence sets `enableThinking: true` with `effort` mapped to `reasoningBudgetTokens` (the runtime teacher-forces the reasoning span closed at the budget); absence disables thinking.

### Model download progress (llama-cpp GGUF / mlx safetensors fetch)

Model downloads report live progress (percent, transferred/total bytes, smoothed speed, ETA) on every surface. Core: `src/core/models/download-progress.ts` is a process-wide pub/sub (`subscribeModelDownloadProgress`/`emitModelDownloadProgress`, plus `formatBytes`/`formatEtaSeconds`/`renderDownloadBar`/`formatModelDownloadLine`/`createModelDownloadLogger` helpers; re-exported from the public API). `src/core/models/hub-download.ts` (shared by the llama-cpp and mlx resolvers) instruments `downloadFileToCacheDir` with a byte-counting `fetch` (only the blob GET is counted — API calls and the `Range: bytes=0-0` probe pass through; cache hits emit nothing; sub-1MiB responses are treated as API chatter, so small metadata/tokenizer files download silently) and passes `xet: false` (honored at runtime, absent from the public type, hence a cast) so the download is one measurable plain-HTTP stream; sharded GGUFs and multi-shard safetensors report per shard with `shard: { index, total }`, emissions are throttled to 250ms and end with a `done`/`error` event. Surfaces: chat renders a transient `DownloadPanel` row in `BottomChrome` (subscribed in `app.tsx`); exec and daemon attach `createModelDownloadLogger` to stderr (in-place TTY line, ~10%-increment log lines when piped; `[daemon] ` prefix for the daemon); the ACP agent forwards events during active turns as a custom `_hoomanjs/model_download` notification (`{ sessionId, ...progress }`; unknown to other clients, ignored per JSON-RPC), which the VS Code extension receives in `acp-client.ts`, routes by session in `chat-view.ts` (`{ type: "download" }` bridge message), and renders as a `DownloadStrip` progress bar above the status strip (cleared on done/error and turn end).

### Model billing metadata (context window + session cost)

Each `llms` entry may carry an optional `billing` block (`src/core/models/types.ts`, `LlmBillingSchema`): `name` (required when the block is present — the models.dev model identifier), optional `context` (window size in tokens), and optional `costs` (`"input/m"`/`"output/m"` required, `"cache/m"` optional; USD per million tokens). When `billing` is omitted, `options.model` is used as the lookup name. Resolution (`src/core/utils/billing.ts`, `resolveLlmBilling`) merges config-provided fields over the models.dev catalog (`https://models.dev/catalog.json`, cached at `~/.hooman/cache/models-dev.json` and refreshed at most once daily, stale-on-failure): model ids are matched with separator normalization plus boundary containment (so `claude-haiku-4.5`, `anthropic/claude-sonnet-4-5`, and Bedrock's region-prefixed ids all resolve), preferring the provider whose id equals the model's canonical lab and falling back to the first/closest matching provider. For local providers (`llama-cpp`, `ollama`) catalog costs are discarded — local inference is free and the catalog prices the hosted API for the same model id — so only the context window resolves and no `$` cost is shown (explicit config `billing.costs` still wins if set). Per-request cost is computed from additive-shape usage (`computeUsageCostUsd`; cache tiers fall back to the input rate) and accumulated at the rates of the model that served each request; once any request with usage runs unpriced, cost reporting stops for the session rather than under-reporting. Context usage is a property of the conversation, not the model: on a model switch the `used` tokens carry over and are rescaled against the new model's window (ACP pushes an immediate `usage_update` on a model config change, and again at the turn boundary after a deferred mid-turn rebuild), while accrued cost is kept. Surfaces: the ACP `usage_update` sends `used` (latest request's additive prompt total), `size` (resolved context, `0` = unknown), and `cost` (only while fully priced) alongside the `_meta["hoomanjs/tokens"]` per-turn token meter (the latest request's `in`/`cin`/`out`, not a session total — the context gauge already covers overall usage) — the VS Code webview renders a context gauge + cost in `UsageFooter.tsx`; the chat TUI's `StatusBar.tsx` middle row shows `context: N% (used/size) • tokens: … • cost: $…` (no `status:`/`turns:` tokens — activity is readable from the transcript; billing resolved on mount and re-resolved on `/model`, tracked via `BillingMeter` in `src/chat/app.tsx`). When nothing resolves (e.g. local Ollama models), neither surface shows context usage or cost.
...
- **stdout discipline:** each CLI surface owns stdout (Ink frame in `chat`, agent output in `exec`, the JSON-RPC stream in `acp`), so third-party `console.*` chatter (e.g. `@huggingface/hub`'s `console.debug` "Downloading …" line) must never reach it. `src/core/utils/logging.ts` provides the helpers, applied per command action in `src/cli.ts`: `redirectLogs()` routes all global console methods to stderr (`exec`, `daemon`, and `acp` — where stdout is the protocol channel and a stray `console.log` would corrupt it); `quietChatLogs()` (for `chat`) drops `console.debug`/`console.info` and silences the Strands SDK logger entirely, because raw stderr writes garble the live Ink frame and Ink's console patch would paint chatter into the transcript. `patchSdkLogger()` at the bottom of `src/cli.ts` is the global default Strands SDK logger (pino, warn+ on stderr). Utility commands (`sessions`, `config`, `mcp`) keep their intentional stdout output.
